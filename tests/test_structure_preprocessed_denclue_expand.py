...
@hide_errors('')
def test_structure_denclue_clustering_denclue_function_function(run_compute):
    function_name = test_structure_denclue_clustering_denclue_function_function
    kwargs = {'student_directory': 'student_code_with_answers' , 'instructor_directory': 'instructor_code_with_answers'}
    correct_answer = run_compute('denclue_clustering', 'denclue_clustering', 'i', **kwargs)
    if 'denclue_function' not in correct_answer:
        explanation = "Key: 'denclue_function' not found in instructor answer!\n"
        test_structure_denclue_clustering_denclue_function_function.explanation = explanation
        assert False
    else:
        correct_answer = correct_answer['denclue_function']
    student_answer = run_compute('denclue_clustering', 'denclue_clustering', 's', **kwargs)
    if 'denclue_function' not in student_answer:
        explanation = "Key: 'denclue_function' not found in student answer!\n"
        test_structure_denclue_clustering_denclue_function_function.explanation = explanation
        assert False
    else:
        student_answer = student_answer['denclue_function']
    local_namespace = {}
    exclude_indices = []
    ...
    question_id = 'denclue_clustering'
    subquestion_id = 'denclue_function'
...
@hide_errors('')
def test_structure_denclue_clustering_plot_original_cluster_scatterplot2d(run_compute):
    function_name = test_structure_denclue_clustering_plot_original_cluster_scatterplot2d
    kwargs = {'student_directory': 'student_code_with_answers' , 'instructor_directory': 'instructor_code_with_answers'}
    correct_answer = run_compute('denclue_clustering', 'denclue_clustering', 'i', **kwargs)
    if 'plot_original_cluster' not in correct_answer:
        explanation = "Key: 'plot_original_cluster' not found in instructor answer!\n"
        test_structure_denclue_clustering_plot_original_cluster_scatterplot2d.explanation = explanation
        assert False
    else:
...
@hide_errors('')
def test_structure_denclue_clustering_cluster_parameters_dict_lbrack_str_comma_float_rbrack(run_compute):
    function_name = test_structure_denclue_clustering_cluster_parameters_dict_lbrack_str_comma_float_rbrack
    kwargs = {'student_directory': 'student_code_with_answers' , 'instructor_directory': 'instructor_code_with_answers'}
    correct_answer = run_compute('denclue_clustering', 'denclue_clustering', 'i', **kwargs)
    if 'cluster parameters' not in correct_answer:
        explanation = "Key: 'cluster parameters' not found in instructor answer!\n"
        test_structure_denclue_clustering_cluster_parameters_dict_lbrack_str_comma_float_rbrack.explanation = explanation
        assert False
    else:
...
@hide_errors('')
def test_structure_denclue_clustering_1st_group_comma_SSE_dict_lbrack_str_comma_float_rbrack(run_compute):
    function_name = test_structure_denclue_clustering_1st_group_comma_SSE_dict_lbrack_str_comma_float_rbrack
    kwargs = {'student_directory': 'student_code_with_answers' , 'instructor_directory': 'instructor_code_with_answers'}
    correct_answer = run_compute('denclue_clustering', 'denclue_clustering', 'i', **kwargs)
    if '1st group, SSE' not in correct_answer:
        explanation = "Key: '1st group, SSE' not found in instructor answer!\n"
        test_structure_denclue_clustering_1st_group_comma_SSE_dict_lbrack_str_comma_float_rbrack.explanation = explanation
        assert False
    else:
...
@hide_errors('')
def test_structure_denclue_clustering_scatterplot_cluster_with_largest_ARI_scatterplot2d(run_compute):
    function_name = test_structure_denclue_clustering_scatterplot_cluster_with_largest_ARI_scatterplot2d
    kwargs = {'student_directory': 'student_code_with_answers' , 'instructor_directory': 'instructor_code_with_answers'}
    correct_answer = run_compute('denclue_clustering', 'denclue_clustering', 'i', **kwargs)
    if 'scatterplot cluster with largest ARI' not in correct_answer:
        explanation = "Key: 'scatterplot cluster with largest ARI' not found in instructor answer!\n"
        test_structure_denclue_clustering_scatterplot_cluster_with_largest_ARI_scatterplot2d.explanation = explanation
        assert False
    else:
        correct_answer = correct_answer['scatterplot cluster with largest ARI']
    student_answer = run_compute('denclue_clustering', 'denclue_clustering', 's', **kwargs)
    if 'scatterplot cluster with largest ARI' not in student_answer:
        explanation = "Key: 'scatterplot cluster with largest ARI' not found in student answer!\n"
        test_structure_denclue_clustering_scatterplot_cluster_with_largest_ARI_scatterplot2d.explanation = explanation
        assert False
    else:
        student_answer = student_answer['scatterplot cluster with largest ARI']
    local_namespace = {}
    exclude_indices = []
    local_namespace['exclude_indices'] = exclude_indices
    include_indices = []
    ...
    answer_type = 'scatterplot2d'
    question_id = 'denclue_clustering'
    subquestion_id = 'scatterplot cluster with largest ARI'
    partial_score_frac_l = [0.]
    local_namespace['partial_score_frac_l'] = partial_score_frac_l
    function_name.answer_type = answer_type
    function_name.question_id = question_id
...
@hide_errors('')
def test_structure_denclue_clustering_scatterplot_cluster_with_smallest_SSE_scatterplot2d(run_compute):
    function_name = test_structure_denclue_clustering_scatterplot_cluster_with_smallest_SSE_scatterplot2d
    kwargs = {'student_directory': 'student_code_with_answers' , 'instructor_directory': 'instructor_code_with_answers'}
    correct_answer = run_compute('denclue_clustering', 'denclue_clustering', 'i', **kwargs)
    if 'scatterplot cluster with smallest SSE' not in correct_answer:
        explanation = "Key: 'scatterplot cluster with smallest SSE' not found in instructor answer!\n"
        test_structure_denclue_clustering_scatterplot_cluster_with_smallest_SSE_scatterplot2d.explanation = explanation
        assert False
    else:
        correct_answer = correct_answer['scatterplot cluster with smallest SSE']
    student_answer = run_compute('denclue_clustering', 'denclue_clustering', 's', **kwargs)
    if 'scatterplot cluster with smallest SSE' not in student_answer:
        explanation = "Key: 'scatterplot cluster with smallest SSE' not found in student answer!\n"
        test_structure_denclue_clustering_scatterplot_cluster_with_smallest_SSE_scatterplot2d.explanation = explanation
        assert False
    else:
        student_answer = student_answer['scatterplot cluster with smallest SSE']
    local_namespace = {}
    exclude_indices = []
    local_namespace['exclude_indices'] = exclude_indices
    include_indices = []
    ...
    answer_type = 'scatterplot2d'
    question_id = 'denclue_clustering'
    subquestion_id = 'scatterplot cluster with smallest SSE'
    partial_score_frac_l = [0.]
    local_namespace['partial_score_frac_l'] = partial_score_frac_l
    function_name.answer_type = answer_type
    function_name.question_id = question_id
...
@hide_errors('')
def test_structure_denclue_clustering_mean_ARIs_float(run_compute):
    function_name = test_structure_denclue_clustering_mean_ARIs_float
    kwargs = {'student_directory': 'student_code_with_answers' , 'instructor_directory': 'instructor_code_with_answers'}
    correct_answer = run_compute('denclue_clustering', 'denclue_clustering', 'i', **kwargs)
    if 'mean_ARIs' not in correct_answer:
        explanation = "Key: 'mean_ARIs' not found in instructor answer!\n"
        test_structure_denclue_clustering_mean_ARIs_float.explanation = explanation
        assert False
    else:
...
@hide_errors('')
def test_structure_denclue_clustering_std_ARIs_float(run_compute):
    function_name = test_structure_denclue_clustering_std_ARIs_float
    kwargs = {'student_directory': 'student_code_with_answers' , 'instructor_directory': 'instructor_code_with_answers'}
    correct_answer = run_compute('denclue_clustering', 'denclue_clustering', 'i', **kwargs)
    if 'std_ARIs' not in correct_answer:
        explanation = "Key: 'std_ARIs' not found in instructor answer!\n"
        test_structure_denclue_clustering_std_ARIs_float.explanation = explanation
        assert False
    else:
...
@hide_errors('')
def test_structure_denclue_clustering_mean_SSEs_float(run_compute):
    function_name = test_structure_denclue_clustering_mean_SSEs_float
    kwargs = {'student_directory': 'student_code_with_answers' , 'instructor_directory': 'instructor_code_with_answers'}
    correct_answer = run_compute('denclue_clustering', 'denclue_clustering', 'i', **kwargs)
    if 'mean_SSEs' not in correct_answer:
        explanation = "Key: 'mean_SSEs' not found in instructor answer!\n"
        test_structure_denclue_clustering_mean_SSEs_float.explanation = explanation
        assert False
    else:
...
@hide_errors('')
def test_structure_denclue_clustering_std_SSEs_float(run_compute):
    function_name = test_structure_denclue_clustering_std_SSEs_float
    kwargs = {'student_directory': 'student_code_with_answers' , 'instructor_directory': 'instructor_code_with_answers'}
    correct_answer = run_compute('denclue_clustering', 'denclue_clustering', 'i', **kwargs)
    if 'std_SSEs' not in correct_answer:
        explanation = "Key: 'std_SSEs' not found in instructor answer!\n"
        test_structure_denclue_clustering_std_SSEs_float.explanation = explanation
        assert False
    else:
